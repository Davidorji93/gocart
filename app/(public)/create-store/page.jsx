...
export default function CreateStore() {
  ...
  const validateForm = () => {
    ...
    if (!storeInfo.image) newErrors.image = "Store logo is required";
    if (!storeInfo.username.trim()) newErrors.username = "Username is required";
    if (!storeInfo.name.trim()) newErrors.name = "Store name is required";
    ...
    } else if (!/^\d{7,15}$/.test(storeInfo.contact)) {
      newErrors.contact = "Contact must be 7–15 digits";
    }

    if (!storeInfo.address.trim()) newErrors.address = "Address is required";
  ...
  const onSubmitHandler = async (e) => {
    ...
    if (!validateForm()) {
      toast.error("Please fix missing fields");
      return;
    }
